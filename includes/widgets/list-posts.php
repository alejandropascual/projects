...
add_action( 'widgets_init', array( 'aps_widget_list_posts', 'aps_register_widget' ) );


class aps_widget_list_posts extends WP_Widget
{
    ...
    {
        $widget_ops = array( 'description' => _x( 'List posts', 'widget', LANGUAGE_THEME ) );
        parent::WP_Widget(false, _x('ELAP: List posts', 'widget', LANGUAGE_THEME), $widget_ops);
    ...
    function widget( $args, $instance )
    {
        ...
        if ($terms_cats)
        {
            ...
            //Render data
            $html .= '<div class="widget-list-posts display-'.$instance['display'].'">';
    ...
    function aps_query_posts( $query_data )
    {
        ...
        //Preparar filtro de category and post_tag
        $tax_query = array( 'relation' => 'AND' );

        if ($query_data['select_cats'] == 'only')
        {
            $query_cats = array();
            $query_cats['category'] = array(
                'taxonomy' => 'category',
                'field' => 'id',
        ...
        //echo '<pre>'; print_r( $query_real ); echo '</pre>';
    ...
    function aps_create_array_data_from_query( $my_query )
    {
        ...
        if ( $my_query->have_posts() ) {
            while ( $my_query->have_posts() ) {
                ...
                $image =  aps_get_image_resized_for_post_id( get_the_ID(), 290, 0, 0.3, 'no');
    ...
    function form( $instance )
    {
        ...
        $terms_tags = get_terms( 'post_tag', array(
            'hide_empty'    => 1,
            'hierarchical'  => false
        ) );
